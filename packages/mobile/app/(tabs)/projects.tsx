...
export default function ProjectsScreen() {
  ...
  if (view === 'site_detail') {
    return (
      <Screen scroll>
        ...
        <Text style={styles.pageTitle}>{selectedSite?.name ?? 'Site'}</Text>
        <Text style={styles.subLabel}>{selectedProject?.name ?? 'Project'}</Text>
    ...
    return (
      <Screen>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.drawingCount}>{filtered.length} drawing{filtered.length !== 1 ? 's' : ''}</Text>
        </View>
        ...
        {disciplines.length > 0 && (
          ...
          >
            <TouchableOpacity
              style={[styles.filterChip, disciplineFilter === null && styles.filterChipActive]}
              onPress={() => setDisciplineFilter(null)}
            >
              <Text style={[styles.filterChipText, disciplineFilter === null && styles.filterChipTextActive]}>
                All
              </Text>
            </TouchableOpacity>
            {disciplines.map((d) => (
              <TouchableOpacity
                ...
                onPress={() => setDisciplineFilter(disciplineFilter === d ? null : d)}
              >
                <Text style={[styles.filterChipText, disciplineFilter === d && styles.filterChipTextActive]}>
                  {d}
                </Text>
              </TouchableOpacity>
            ))}
              ...
              return (
                ...
                >
                  <Card style={styles.drawingCard}>
                    {/* Row 1: drawing number + discipline */}
                    <View style={styles.drawingCardTop}>
                      <Text style={styles.drawingNumber} numberOfLines={1}>
                        {item.drawingNumber}
                      </Text>
                      {item.discipline && (
                        <Badge label={item.discipline} variant="outline" />
                      )}
                    </View>
                    ...
                    {/* Row 3: revision badges + date */}
                    <View style={styles.drawingCardBottom}>
                      <View style={styles.revBadgeRow}>
                        {rev ? (
                          <>
                            <Badge label={`Rev ${rev.revisionNumber}`} variant="default" />
                            <Badge
                              label={rev.status.replace(/_/g, ' ')}
                              variant={REVISION_STATUS_VARIANT[rev.status] ?? 'default'}
                            />
                          </>
                        ) : (
                          <Badge label="No revision" variant="outline" />
                        )}
                      </View>
                      {dateStr && (
                        <Text style={styles.revDate}>{dateStr}</Text>
                      )}
                    </View>
                  </Card>
                </TouchableOpacity>
  ...
  if (view === 'drawing_detail' && selectedDrawing) {
    ...
    async function openPdf() {
      ...
      // Open from local cache if already downloaded.
      ...
      try {
        ...
        if (result.status !== 200) {
          Alert.alert('Error', 'Could not download the drawing PDF.');
          return;
        }
        ...
        Alert.alert(
          'PDF not available',
          'PDF not yet downloaded. Connect to the internet to download it first.',
        );
      }
    }
      ...
      <Screen scroll>
        ...
        <Card style={[styles.summaryCard, { marginTop: 16 }]}>
          ...
          {selectedDrawing.discipline && (
            <InfoRow label="Discipline" value={selectedDrawing.discipline} />
          )}
        </Card>
        ...
        {rev ? (
          <Card style={[styles.summaryCard, { marginTop: 12 }]}>
            ...
            <Button title="Open Drawing PDF" onPress={openPdf} style={{ marginTop: 12 }} />
          </Card>
        ) : (
          <Card style={[styles.summaryCard, { marginTop: 12 }]}>
          ...
          <Card style={[styles.summaryCard, { marginTop: 12 }]}>
            ...
            <View style={styles.statusBtnRow}>
              {instr.status !== 'acknowledged' && (
                <Button
                  title="Acknowledge"
                  onPress={() => void updateStatus('acknowledged')}
                  variant="secondary"
                  style={{ flex: 1 }}
                />
              )}
